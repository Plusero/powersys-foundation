...
Consider the per-unit branch data below. The tap is located at the listed from bus, and each $b_c$ is the total line-charging susceptance.

| From | To | $z=r+\mathrm{j}x$ | $b_c$ | $t=\tau e^{\mathrm{j}\phi}$ |
|---:|---:|---:|---:|---:|
| 1 | 2 | $0.02+\mathrm{j}0.06$ | $0.030$ | $1.05e^{\mathrm{j}5^\circ}$ |
| 1 | 3 | $0.08+\mathrm{j}0.24$ | $0.025$ | $1$ |
| 2 | 3 | $0.06+\mathrm{j}0.18$ | $0.020$ | $1$ |

: Three-bus branch data {#tbl-ybus-three-bus-data}

Bus 3 also has the fixed shunt $y_3^{\mathrm{sh}}=\mathrm{j}0.010$. Stamping the three primitives and then the bus shunt gives

$$
\mathbf Y_{\mathrm{bus}}\approx
\begin{bmatrix}
 5.78515-\mathrm{j}17.32934 & -5.98887+\mathrm{j}13.81633 & -1.25000+\mathrm{j}3.75000\\
-3.49870+\mathrm{j}14.64638 &  6.66667-\mathrm{j}19.97500 & -1.66667+\mathrm{j}5.00000\\
-1.25000+\mathrm{j}3.75000 & -1.66667+\mathrm{j}5.00000 &  2.91667-\mathrm{j}8.71750
\end{bmatrix}.
$$ {#eq-ybus-three-bus-matrix}

Notice that $Y_{12}\ne Y_{21}$. This is the expected consequence of the $5^\circ$ phase-shifting tap, not an assembly mistake.
...
branches = [
    {
        ...
        "charging_susceptance": 0.030,
        "tap": 1.05 * np.exp(1j * np.deg2rad(5.0)),
    },
    {
        "from_bus": 1,
        "to_bus": 3,
        "impedance": 0.08 + 0.24j,
        "charging_susceptance": 0.025,
    },
    {
        "from_bus": 2,
        "to_bus": 3,
        "impedance": 0.06 + 0.18j,
        "charging_susceptance": 0.020,
    },
]

bus_shunts = np.array([0.0j, 0.0j, 0.010j])
y_bus = build_y_bus(3, branches, bus_shunts)
...
assembled_current += bus_shunts * voltage
...
where $\odot$ denotes elementwise multiplication. The small positive total active injection in this passive example is the network's resistive loss. Reactive totals also include the effects of line charging and the fixed shunt.

@Eq-ybus-power-injection is the bridge from network assembly to nonlinear power-flow equations. The [Newton--Raphson power-flow tutorial](newton-raphson-power-flow.qmd) develops that next step. Keep the primitive branch matrices as well as $\mathbf Y_{\mathrm{bus}}$ when individual terminal currents or branch power flows are required.
