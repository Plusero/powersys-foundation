...
### 2.1 Why derivatives become multiplication by $\mathrm{j}\omega$

Differentiating the complex exponential gives

$$
\frac{\mathrm d}{\mathrm dt}
\left(\underline V e^{\mathrm{j}\omega t}\right)
=\mathrm{j}\omega\underline V e^{\mathrm{j}\omega t}.
$$

The time-domain derivative therefore corresponds to multiplying the phasor by $\mathrm{j}\omega$. Multiplication by $\mathrm{j}=e^{\mathrm{j}\pi/2}$ also rotates a complex number counter-clockwise by $90^\circ$. These two facts produce the familiar inductor and capacitor impedances.
