...
Thus, $L_{\mathrm{eq}}<\min(L_1,\ldots,L_n)$: an uncoupled parallel combination has a smaller inductance than any individual inductor. Initial branch currents affect the total initial current but not the reciprocal relationship between subsequent changes in voltage and current.

If the flux from one coil links another, mutual inductance terms must be included. In that case, simply adding or reciprocally combining the self-inductances can be wrong.
...
The inductor consumes zero active power because it stores energy during part of every cycle and returns it during the rest of the cycle. Because inductive reactance produces lagging current, inductors absorb reactive power from the network. See [power factor](power-factor-in-ac-circuits.qmd) for how this exchange affects an AC system.
...
A common first-order equivalent circuit places winding resistance in series with the ideal inductance and parasitic capacitance across the two terminals, as shown in @fig-practical-inductor-model.

![First-order practical inductor model. Winding resistance is in series with the ideal inductance, and parasitic capacitance shunts the series branch.](figures/generated/practical-inductor-model.svg){#fig-practical-inductor-model fig-align="center" width="85%"}
...
- **parasitic capacitance**, which creates a self-resonant frequency;
