...
You should be comfortable with RMS phasors, complex power, per-unit quantities, and nodal network equations. The [complex-numbers tutorial](complex-numbers-in-ac-circuits.qmd) reviews phasors and complex power, the [per-unit tutorial](per-unit-values.qmd) explains the normalisation used in practical network models, and the [Y-bus tutorial](y-bus-matrix.qmd) develops admittance stamping and the nodal equation $\mathbf I=\mathbf Y_{\mathrm{bus}}\mathbf V$. Basic familiarity with partial derivatives and linear systems will help with the Newton step. Python and NumPy are used for the implementation, and Power Grid Model is used for an independent numerical cross-check.
...
@fig-three-bus-power-flow-single-line shows the meshed topology, bus roles, and series impedances used in the worked example. It uses IEC-style generator, busbar, and load symbols.
...
The unknown state is $\mathbf x=[\theta_2,\theta_3,|V_3|]^{\mathsf T}$. At every iteration, this example therefore solves the explicit linear system
...
There is no $Q_2$ row because bus 2 is a PV bus, and there is no $|V_2|$ correction because its voltage magnitude is specified. The first cell stamps a series-only Y-bus matrix and defines the retained calculated injections in the row order of @eq-three-bus-newton-system.

::: {.callout-tip}
### Recalculate the example locally

The published tutorial is a static page, so its Python cells do not execute in the browser. To change the inputs and recalculate both solutions, first install [Quarto](https://quarto.org/docs/get-started/) and [`uv`](https://docs.astral.sh/uv/), and then run
...
Edit the block marked `USER INPUTS` below and save the file. Quarto will execute the three Python cells in order and refresh the results in the browser. For a one-time calculation without the live preview, replace the last command with `uv run quarto render articles/newton-raphson-power-flow.qmd`.
:::
...
# USER INPUTS: change these values and rerun the tutorial.
slack_voltage_pu = 1.04
slack_angle_rad = 0.0
pv_active_injection_pu = 0.50
pv_voltage_pu = 1.01
pq_active_injection_pu = -0.60
pq_reactive_injection_pu = -0.25
...
def voltage_from_state(state: np.ndarray) -> np.ndarray:
    ...
    angles = np.array([slack_angle_rad, state[0], state[1]])
    magnitudes = np.array([slack_voltage_pu, pv_voltage_pu, state[2]])
...
specified = np.array(
    [
        pv_active_injection_pu,
        pq_active_injection_pu,
        pq_reactive_injection_pu,
    ]
)
```

The next cell evaluates @eq-jacobian-off-diagonal and @eq-jacobian-diagonal directly. The index arrays then select the reduced Jacobian implied by the bus types: buses 2 and 3 contribute $P$--$\theta$ equations, while only bus 3 contributes the $Q$--$|V|$ equation.

```{python}
#| label: lst-nr-three-bus-solve
#| code-summary: "Assemble the analytical Jacobian and solve the three-bus power flow"

def power_flow_jacobian(state: np.ndarray) -> np.ndarray:
    """Return the reduced analytical Jacobian for [P2, P3, Q3]."""
    voltage = voltage_from_state(state)
...
state = np.array([slack_angle_rad, slack_angle_rad, initial_pq_voltage_pu])
tolerance = 1e-10
...
The iteration starts from the flat profile, drives the retained residual below the tolerance, and leaves the slack-bus and PV-bus reactive injections as calculated outputs. This is the numerical counterpart of the bus-type bookkeeping in @tbl-bus-types.

### Cross-check with Power Grid Model

We can now solve the same physical network with the [`PowerGridModel.calculate_power_flow`](https://power-grid-model.readthedocs.io/en/stable/examples/Power%20Flow%20Example.html#one-time-power-flow-calculation) function and compare the result with our implementation. Power Grid Model accepts physical SI quantities, so choose coherent bases
...
pgm_generator[AttributeType.p_specified] = [pv_active_injection_pu * s_base]
...
pgm_load[AttributeType.p_specified] = [-pq_active_injection_pu * s_base]
pgm_load[AttributeType.q_specified] = [-pq_reactive_injection_pu * s_base]
...
np.testing.assert_allclose(
    pgm_voltage_magnitude, manual_voltage_magnitude, atol=1e-10, rtol=0.0
)
np.testing.assert_allclose(
    pgm_voltage_angle, manual_voltage_angle, atol=1e-10, rtol=0.0
)
np.testing.assert_allclose(pgm_injection, manual_injection, atol=1e-10, rtol=0.0)
```

The two solutions agree to floating-point precision. This verifies the solved voltages as well as the calculated slack-bus injection and PV-bus reactive injection; comparing only the specified $P_2$, $P_3$, and $Q_3$ values would be a much weaker test.

::: {.callout-tip}
Change the PQ load, the initial voltage, or a line impedance and rerun the tutorial. Watching the residual history and solved voltages change is a quick way to develop intuition about loading, initialisation, and network strength. The table and single-line diagram describe the original case and do not change when the code inputs are edited.
:::
